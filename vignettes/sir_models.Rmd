...
dt = 0.25
n_particles <- 10L
sir_model$reset(data = list(dt = 0.25, 
                            S_ini = 2000, 
                            I_ini = 10, 
                            beta = 0.4, 
                            gamma = 0.1),
...
n_particles <- 5L
model <- gen_age$new(data = list(dt = 0.25, 
...
for (i in 1:N_age) {
  par(mar = c(3, 4, 2, 0.5))
  cols <- c(S = "#8c8cd9", I = "#cc0044", R = "#999966")
  matplot(time, t(x[i,, ]), type = "l",
          xlab = "", ylab = "", yaxt="none", main = paste0("Age ", contact$demography$age.group[i]),
          col = cols[["S"]], lty = 1, ylim=range(x))
  matlines(time, t(x[i+N_age, , ]), col = cols[["I"]], lty = 1)
  matlines(time, t(x[i+2*N_age, , ]), col = cols[["R"]], lty = 1)
